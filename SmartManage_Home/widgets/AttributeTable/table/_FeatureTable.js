...
define([
  ...
  Tooltip
    ...
    return declare([_WidgetBase, Evented], {
      ...
      BulkUpdateDialog: function (title, content) {

        var jsonArr = [];
        for (var i = 0; i < this.layer.fields.length; i++) {
          jsonArr.push({
            id: i,
            name: this.layer.fields[i].name,
            type: this.layer.fields[i].type,
            domain: this.layer.fields[i].domain,
            domstore: this.domainstore
          });
        }
        this.featuresstore.setData([]);
        this.featuresstore.setData(jsonArr);
        if (dijit.byId('ddlattributes')) {
          dijit.byId('ddlattributes').destroy();
          dijit.byId('ddldomaincombobox').destroy();
          dijit.byId('txtdomain').destroy();
          dijit.byId('myarea').destroy();
          dijit.byId('date1').destroy();
          dijit.byId('bulkDialog').destroy();
          dijit.byId('bulkButton').destroy();
        }
        if (!dijit.byId('ddlattributes')) {
          var myComboBox = new ComboBox({ id: "ddlattributes", name: "Feature", value: "", store: this.featuresstore, searchAttr: "name", style: "width:100%; color:Black;  height: 30px;" }, "ddlattributes");
          var mydomainComboBox = new ComboBox({ id: "ddldomaincombobox", name: "domain", value: "", store: this.domainstore, searchAttr: "name", style: "width:100%; color:Black;  height: 30px;" }, "ddldomaincombobox");
          var mydomaintext = new dijit.form.TextBox({ id: "txtdomain", name: "txtdomain", value: "", style: "width:100%;height:30px;" }, "txtdomain");
          var selectionchange = dijit.byId('ddlattributes');

          selectionchange.on('change', lang.hitch(function () {
            if (this.item.domain != undefined) {
              dojo.style(dijit.byId(ddltextDiv), { 'display': 'none' });
              dojo.style(dijit.byId(ddldateDiv), { 'display': 'none' });
              dojo.style(dijit.byId(ddldomainDiv), { 'display': 'block' });
              this.item.domstore.setData([]);
              this.item.domstore.setData(this.item.domain.codedValues);
            }
            else {
              if (this.item.type == "esriFieldTypeDate") {
                dojo.style(dijit.byId(ddldomainDiv), { 'display': 'none' });
                dojo.style(dijit.byId(ddldateDiv), { 'display': 'block' });
                dojo.style(dijit.byId(ddltextDiv), { 'display': 'none' });
              }
              else {
                dojo.style(dijit.byId(ddldomainDiv), { 'display': 'none' });
                dojo.style(dijit.byId(ddldateDiv), { 'display': 'none' });
                dojo.style(dijit.byId(ddltextDiv), { 'display': 'block' });
              }
            }
          ...
          var myButton = new Button({
            ...
            onClick: lang.hitch(this, function () {
              ...
              var bulkupdatevalue;
              if (dijit.byId('myarea').value == "") {
                alert("Please Enter IDs, With Coma Separated");
              }

              selectedfeatureurl = this.layer.url;
              var selectedids = dijit.byId('myarea').value.split(',');

              if (dijit.byId('ddlattributes').item != null) {
                selectedattribute = dijit.byId('ddlattributes').item.name;
              }

              if (dijit.byId('ddldomaincombobox').item != null) {
                bulkupdatevalue = dijit.byId('ddldomaincombobox').item.code;
              }
              else if (dojo.byId("date1").value != "") {
                bulkupdatevalue = dojo.byId("date1").value;
              }
              else {
                bulkupdatevalue = dijit.byId('txtdomain').value;
              }
              if ((dijit.byId('ddlattributes').item.name == "CHGCODE") && (bulkupdatevalue=="1"))
              {
                alert("Please Check You are enterd wrong CHGCODE to New Features");
                return false;
              }
              var updateobject = [];
              ...
              query.outFields = ["*"];
              query.where = "ID in(" + dijit.byId('myarea').value + ")";
              ...
              queryTask.execute(query, lang.hitch(this, function (resultfeat) {
                ...
                for (var j = 0; j < features.length; j++) {
                  ...
                  graphic_upd.attributes[selectedattribute] = bulkupdatevalue;
                  updateobject.push(graphic_upd);

                }
                this.layer.applyEdits(null, updateobject, null);
                dojo.byId(date1).value = "";
                dijit.byId(txtdomain).value = "";
                dijit.byId(ddldomaincombobox).item = null;
                dijit.byId(myarea).value = "";
                on(this.layer, "edits-complete", lang.hitch(this, function () {
                  this.myDialog.hide();
                  this.layer.refresh();
                  this.grid.refresh();
                }))
          ...
          this.myDialog = new Dialog({
            id: 'bulkDialog',
            title: "Bulk Update",
            content: '<div><label for="name">IDs:</label><div id="txtareaid"><textarea id ="myarea"></textarea></div></div><br>' +
            '<div><label for="name">Attribute:</label><div id="ddlattributesDiv"></div></div><br>' +
            '<div><label for="name">Value: </label><div id="ddldomainDiv" style="display:none"></div>' +
            '<div id="ddltextDiv"></div>' +
            '<div id="ddldateDiv" style="display:none"><input type="text" name="date1" id="date1" value="" style: "width:100%;height:30px;" data-dojo-type="dijit/form/DateTextBox" required="true" /></div></div><br>' +
            //'<label for="name">Value:</label><input data-dojo-type="dijit/form/TextBox" value="" id="txtchangevalue" name="name"><br>' + '<br>' +
            '<div id="updatebutton" style="float: right;"> </div>',
            style: "width: 300px; height: 400px;"
          });
          var mytextarea = new Textarea({ name: "myarea", value: "", style: "width:100%;height:150px;max-height:150px" }, "myarea");

          myComboBox.placeAt(ddlattributesDiv);
          myButton.placeAt(updatebutton);
          mydomainComboBox.placeAt(ddldomainDiv);
          mydomaintext.placeAt(ddltextDiv);
          // dojo.byId(date1).value = "";
          // dijit.byId(txtdomain).value = "";
          // dijit.byId(ddldomaincombobox).item = null;
          // dijit.byId(myarea).value = "";
          dijit.byId(myarea).placeholder = "e.g. 12345,67891,..."
          require(["dijit/Tooltip", "dojo/domReady!"], function (Tooltip) {
            new Tooltip({
              connectId: ["myarea"],
              label: "Please Enter IDs With Comma Separated without Spaces"
            });
          });
      ...
      createToolbar: function () {
        ...
        this.ShowallFilterButton = new Button({
          label: this.nls.ShowFilteredLayers,
          iconClass: "esriAttributeTableClearImage",
          onClick: lang.hitch(this, this.ShowFilteredLayers)
        });
        toolbar.addChild(this.ShowallFilterButton);
        ...
        this.BulkUpdatefields = new Button({
          label: this.nls.BulkUpdate,
          iconClass: "esriAttributeTableUpdateImage",
          onClick: lang.hitch(this, this.BulkUpdate)
        });
        toolbar.addChild(this.BulkUpdatefields);
      ...
      changeToolbarStatus: function () {
        ...
        if ((this._filterObj == "") || (this._filterObj == null)) {
      ...
      ShowFilteredLayers: function () {
        ...
        // });
        for (i = 0; i < _filterManager.layerInfos._finalLayerInfos.length; i++) {
          _filterManager.layerInfos._finalLayerInfos[i].setFilter("1=1");
          this._filterObj = "";
        }


        for (i = 0; i < _filterManager.layerInfos._finalTableInfos.length; i++) {
          _filterManager.layerInfos._finalTableInfos[i].setFilter("1=1");
        }
      },

      clearFilter: function () {
        this.layerInfo.setFilter("1=1");
        this._filterObj = "";
      },

      BulkUpdate: function () {

        this.BulkUpdateDialog("Heading ", "Content.");
