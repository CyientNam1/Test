...
define([
  ...
  domClass, domStyle, NlsStrings, DirectionalLineSymbol, Color, SimpleLineSymbol, SimpleRenderer) {
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
      ...
      FlagUGPrimary: true,
      FlagUGSecondary: true,
      FlagUGService: true,
      FlagOHPrimary: true,
      FlagOHSecondary: true,
      FlagUGService: true,
      ...
      _onPopupMenuClick: function (layerInfo, popupMenu, layerTrNode, evt) {
        ...
        evt.stopPropagation();
        this._DirectionCheck(layerInfo, popupMenu);
      },
      _DirectionCheck: function (layerInfo, popupMenu) {
        // alert("Hi");
        if (layerInfo.title == "UG Primary" || layerInfo.title == "UG Secondary" || layerInfo.title == "UG Service"
          || layerInfo.title == "OH Primary" || layerInfo.title == "OH Secondary" || layerInfo.title == "OH Service") {

            if (layerInfo.directionFlag == undefined ) {
            layerInfo.directionFlag = false;
            var ckDirectionDiv = domConstruct.create('div', {
              'class': 'menu-item-identification'
            }, popupMenu.dropMenuNode);

            var ckDirectionSeperate = domConstruct.create('hr', {
              'class': 'menu-item-identification menu-item-line'
            }, ckDirectionDiv, 'before');

            var ckDirection = new CheckBox({
              checked: false,
              'class': "",
              'id': 'chk' + layerInfo.title,
              'style': "margin-left: 24px; margin-top: 10px;",
              label: "Direction",
              onChange: lang.hitch(layerInfo, function (b) {
                if (b) {
                  var pgOptions = {
                    style: this.layerObject.renderer.defaultSymbol.style,
                    color: this.layerObject.renderer.defaultSymbol.color,
                    width: this.layerObject.renderer.defaultSymbol.width,
                    directionSymbol: "arrow2",
                    directionPixelBuffer: 30,
                    directionColor: this.layerObject.renderer.defaultSymbol.color,
                    directionScale: 0.8,
                    showStartSymbol: true,
                    showEndSymbol: true
                  };
                  var pgSymbol = new DirectionalLineSymbol(pgOptions);
                  var renderer = new SimpleRenderer(pgSymbol);
                  this.originalRenderer = this.layerObject.renderer;

                  this.layerObject.setRenderer(renderer);
                  this.layerObject.refresh();
                } else {
                  this.layerObject.setRenderer(this.originalRenderer);
                  this.layerObject.refresh();
                }
              })
            });
            ckDirection.labelNode.style = 'display: unset;padding: unset;font-size: unset;font-weight: unset;line-height: unset;color: unset;text-align: unset; white-space: unset;vertical-align: unset;border-radius: unset; padding-top: 3px;'

            domConstruct.place(ckDirection.domNode, ckDirectionDiv);
          }
        }
