...
define([
  ...
  mo.mergeFieldInfosWithConfiguration = function (layerInfo, configurationLayerInfo) {
    var fieldInfos = [];
    var defaultEditableFieldInfos = this.getDefaultEditableFieldInfos(configurationLayerInfo, true);
  ...
  mo.getDefaultEditableFieldInfos = function (layerInfo, editableOnly) {
    ...
    var fieldInfos = layerInfo.fieldInfos; //  this.getFieldInfosFromWebmap(layerInfo);
  ...
  mo.getConfigInfos = function (jimuLayerInfos, configurationLayerInfos, editableOnly, configuredOnly) {
    ...
    // array.forEach(jimuLayerInfos.getLayerInfoArrayOfWebmap(), function (layerInfo) {
      array.forEach(jimuLayerInfos.getTableInfoArray(), function (layerInfo) {
      var addLayer = false;
      // if (layerInfo.layerObject.type === "Feature Layer" && layerInfo.layerObject.url) {
        if (layerInfo.layerObject.url) {
        if (editableOnly) {//if (layerInfo.layerObject.isEditable && layerInfo.layerObject.isEditable() && editableOnly) {
          addLayer = true;
        }
        ...
        if (mergedConfigInfo.featureLayer.layerAllowsDelete === true &&
          ...
          if (configuredOnly && configuredOnly === true) {
            var layerPartOfConfig = array.some(configurationLayerInfos, function (configurationLayerInfo) {
              return configurationLayerInfo.featureLayer.id === mergedConfigInfo.layerInfo.id;//return configurationLayerInfo.featureLayer.id === mergedConfigInfo.featureLayer.id;
            });
  ...
  mo.getConfigInfo = function (layerInfo, configurationLayerInfos) {
    ...
    var found = array.some(configurationLayerInfos, function (configurationLayerInfo) {
      if (configurationLayerInfo.featureLayer && configurationLayerInfo.featureLayer.id === layerInfo.id) {//if (configurationLayerInfo.featureLayer && configurationLayerInfo.featureLayer.id === layerInfo.layerObject.id) {
  ...
  mo.createDefaultConfigInfo = function (layerInfo) {
    ...
    var configInfo = {
      ...
      'allowUpdateOnly': !allowsCreate,
      'allowDelete': false,
      'fieldInfos': this.mergeFieldInfosWithConfiguration(layerInfo, layerInfo),
      '_editFlag': false
    };
    return configInfo;
  };
