define([
  ...
  function (
    ...
    document.getElementById(this.id).style.right = "0px";
    document.getElementById(this.id).style.bottom = "0px";
    document.getElementById(this.id).style.width = "100%";
    document.getElementById(this.id).style.zIndex = "-1";
    document.getElementById(this.id).style.position = "absolute";
    document.getElementById(this.id).style.borderLeft = "4px solid black";
    dojo.query("div#map").style("background-color", "rgba(255, 255, 255, 0)");
    dojo.query("div#map_root").style("width","50%");
    dojo.query("div.jimu-anchorbar-controller").style("left", dojo.query("div.jimu-anchorbar-controller").style("left")[0]/2 + "px");
        // var AutoCheckPreElectriclayer = array.filter(this.map.layerIds, lang.hitch(this, function (layerId) {
        //   if (layerId.toUpperCase().startsWith('XCEL_APP_TEST_ELECTRIC')) {
        ...
        // var AutoCheckPreElectriclayer = array.filter(this.map.layerIds, lang.hitch(this, function (layerId) {
        //   if (layerId.toUpperCase().startsWith('XCEL_APP_TEST_REF')) {
        //     return this.map.getLayer(layerId);
        ...
        if (this.map != null) {
          ...
          this.originalMapResizeEvent = this.map.on("resize", lang.hitch(this, function (){
            dojo.query("div.jimu-anchorbar-controller").style("left", dojo.query("div.jimu-anchorbar-controller").style("left")[0] / 2 + "px");
          }));
        }
        // this.map.setExtent(this.map.extent);
        if (this.duMap != null){
          this.duMap.getLayer("XCEL_PREELECTRIC_6267").title = "Dual Map Layer";
          this.map.addLayer(this.duMap.getLayer("XCEL_PREELECTRIC_6267"));
          this.duMap.removeLayer(this.duMap.getLayer("XCEL_PREELECTRIC_6267"));
           this.duMap.addLayer(this.map.getLayer("XCEL_PREELECTRIC_6267"));
          }
      ...
      onClose: function () {
        ...
        // dojo.query("div#node1").style("width","0%");
        dojo.query("div#map_root").style("width","100%");
        ...
        if (this.originalMapExtChangeEvent) {
          ...
          this.originalMapResizeEvent.remove();
        }
        this.map.removeLayer(this.duMap.getLayer("XCEL_PREELECTRIC_6267"));
