    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _setOptionsOfSwipeLayers: function (layerInfos) {
        ...
        if (data && data.length > 0) {
          var add = [];
          array.forEach(data, lang.hitch(this, function (layer) {
            if (layer.value === this._currentLayerId)
              add = [layer];// CYIENT CODE
          }));

          this.swipeLayers.set('options', add);
        } else {
      ...
      onOpen: function () {
        ...
        for (var i = 0; i < layers.length; i++) {
          ...
          if (layers[i].id == this.config.layer) {
            this.SwipeLayer = layers[i].layerObject;
          }
        }
