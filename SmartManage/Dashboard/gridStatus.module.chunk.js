...
var GridStatusComponent = (function () {
    function GridStatusComponent(exportToCsvService, authenticationService) {
    ...
    GridStatusComponent.prototype.getKpiData = function () {
        ...
        sheet.getUnderlyingDataAsync(options).then((function (t) {
            _this.authenticationService.getTableauToken();
            var kpiData = t.getData();
            // t.getData().forEach(item => {
            //     if (item[0].value.includes("total")) {
            //         this.totalFeedersArray.push({ "id": item[0].value.split('_')[0] });
            //     }
            //     if (item[0].value.includes("delivered")) {
            //         this.deliveredArray.push({ "id": item[0].value.split('_')[0] });
            //     }
            //     if (item[0].value.includes("backoffice")) {
            //         this.backOfficeArray.push({ "id": item[0].value.split('_')[0] });
            //     }
            //     if (item[0].value.includes("inprogress")) {
            //         this.fcInProgressArray.push({ "id": item[0].value.split('_')[0] });
            //     }
            // });
            _this.itemsTotal = kpiData.filter(function (x) { return x[0].value.indexOf('total') > -1; }).map(function (y) {
                return { "id": y[0].value.split('_')[0] };
            });
            _this.itemsDelivered = kpiData.filter(function (x) { return x[0].value.indexOf('delivered') > -1; }).map(function (y) {
                return { "id": y[0].value.split('_')[0] };
            });
            ;
            _this.itemsBackOffice = kpiData.filter(function (x) { return x[0].value.indexOf('backoffice') > -1; }).map(function (y) {
                return { "id": y[0].value.split('_')[0] };
            });
            ;
            _this.itemsInProgress = kpiData.filter(function (x) { return x[0].value.indexOf('inprogress') > -1; }).map(function (y) {
                return { "id": y[0].value.split('_')[0] };
            });
            ;
            // this.itemsTotal = this.totalFeedersArray;
            // this.itemsDelivered = this.deliveredArray;
            // this.itemsBackOffice = this.backOfficeArray;
            // this.itemsInProgress = this.fcInProgressArray;
            _this.totalFeeders = _this.itemsTotal.length;
            _this.delivered = _this.itemsDelivered.length;
            _this.backOffice = _this.itemsBackOffice.length;
            _this.fcInProgress = _this.itemsInProgress.length;
        }));
    };
    GridStatusComponent.prototype.ngOnInit = function () {
        ...
        $.ajax({
            ...
            success: (function (data) {
                ...
                console.log(data);
                var completionUrl = 'https://busrep.cyient.com/trusted/' + data + '/views/LandingPage/Dashboard1?:embed=y&:showAppBanner=false&:showShareOptions=true&:display_count=no&:showVizHome=no';
                ...
                _this.tableauViz = new tableau.Viz(placeholderDiv, completionUrl, options);
            }),
            error: function (data) {
                return data;
            }
    ...
    GridStatusComponent.prototype.getKPIdata = function () {
        ...
        $.ajax({
            ...
            success: (function (data) {
                ...
                console.log(data);
                var testGetDataURL = 'https://busrep.cyient.com/trusted/' + data + '/views/KPINEW/kpiData';
                _this.tFeedersViz = new tableau.Viz(containerDiv, testGetDataURL, {
                    hideTabs: true,
                    width: 0,
                    height: 0,
                    onFirstInteractive: (function () {
...
GridStatusComponent = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof __WEBPACK_IMPORTED_MODULE_1__core_services_export_service__["a" /* ExportToCsvService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_1__core_services_export_service__["a" /* ExportToCsvService */]) === "function" && _a || Object, typeof (_b = typeof __WEBPACK_IMPORTED_MODULE_2__core_authentication_authentication_service__["a" /* AuthenticationService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_2__core_authentication_authentication_service__["a" /* AuthenticationService */]) === "function" && _b || Object])
], GridStatusComponent);
