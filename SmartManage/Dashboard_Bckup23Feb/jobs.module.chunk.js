...
/***/ "../../../../../src/app/routes/jobs/feeder-details/feeder-details.component.scss":
...
/***/ "../../../../../src/app/routes/jobs/feeder-details/feeder-details.component.ts":
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return FeederDetailsComponent; });
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__esriMap_esriMap_esri_map_component__ = __webpack_require__("../../../../../src/app/routes/esriMap/esriMap/esri-map.component.ts");
...
var FeederDetailsComponent = (function () {
    function FeederDetailsComponent(_router, _jobService) {
        this._router = _router;
        this._jobService = _jobService;
        this.jobsExecutedForFeeder = [];
        this.errorDetailsForFeeder = [];
        this.jobResultsForFeeder = [];
        this.kpiData = {
        ...
        this.errorData = [];
        this.header = 'Feeder Details';
    }
    FeederDetailsComponent.prototype.ngOnInit = function () {
        this.getFeederDetails();
    };
    FeederDetailsComponent.prototype.getFeederDetails = function () {
        var _this = this;
        this.selectedFeeder = this._router.snapshot.paramMap.get('id');
        this._jobService.getJobs().subscribe(function (res) {
            _this.filterJobsByFeeder(res.fiopsJobs);
        });
    };
    FeederDetailsComponent.prototype.filterJobsByFeeder = function (data) {
        var _this = this;
        var jobsExecuted = [];
        data.forEach(function (element) {
            var desc = element.description.split('@');
            var feederOrGrid = desc.length > 1 ? desc[1].split(':')[1] : '';
            if (feederOrGrid === _this.selectedFeeder) {
                jobsExecuted.push(element);
            }
        });
        console.log(jobsExecuted);
        this.jobsExecutedForFeeder = jobsExecuted;
        if (jobsExecuted.length > 0) {
            this.getFeederJobDetails(jobsExecuted);
        }
    };
    FeederDetailsComponent.prototype.getFeederJobDetails = function (jobsExcuted) {
        var _this = this;
        var count = 0, jobResults = [];
        jobsExcuted.forEach(function (element) {
            _this._jobService.getReportDetails(element.fiopsJobID).subscribe(function (res) {
                count++;
                jobResults.push(res);
                if (count === jobsExcuted.length) {
                    _this.processData(jobResults);
                }
            }, function (error) {
                count++;
                if (count === jobsExcuted.length) {
    ...
    FeederDetailsComponent.prototype.processData = function (jobResults) {
        var _this = this;
        this.errorDetailsForFeeder = jobResults;
        this._jobService.getFeederJobs(this.jobsExecutedForFeeder.map(function (x) { return x.fiopsJobID; })).subscribe(function (res) {
            _this.jobResultsForFeeder = __WEBPACK_IMPORTED_MODULE_3_lodash__["sortBy"](res, "fiops_job_id").reverse();
            console.log('job Results: ');
            console.log(_this.jobResultsForFeeder);
            _this.filterKPIData();
        });
        console.log('error details: ');
        console.log(jobResults);
    };
    FeederDetailsComponent.prototype.filterKPIData = function () {
        var rulebaseIds = [];
        var filteredData = [];
        this.jobResultsForFeeder.forEach(function (item) {
            if (rulebaseIds.indexOf(item.rb_id) === -1) {
                rulebaseIds.push(item.rb_id);
        ...
        var kpiData = {
            featuresValidated: 0,
            errorsFound: 0,
            rulesExecuted: rulebaseIds.length
        };
        filteredData.forEach(function (el) {
            kpiData.featuresValidated += +el.features_read;
            kpiData.errorsFound += +el.errors_written || 0;
        });
        this.kpiData.numOfRules = kpiData.rulesExecuted;
        this.kpiData.numOfErrors = kpiData.errorsFound;
        this.kpiData.numOfValidatedFeatures = kpiData.featuresValidated;
        console.log(kpiData);
        console.log(filteredData);
        this.processErrorData(filteredData);
    };
    FeederDetailsComponent.prototype.processErrorData = function (filteredData) {
        var errorData = [];
        var feederJobsWithErrors = this.errorDetailsForFeeder.filter(function (x) { return x.features.length > 0; });
        feederJobsWithErrors.forEach(function (element) {
            element.features.forEach(function (feature) {
                filteredData.forEach(function (item) {
                    if (feature.properties.JobID == item.fiops_job_id && feature.properties.RuleBaseID == item.rb_id) {
                        errorData.push(feature);
                    }
                });
            });
        });
        console.log('Error Data Filterd: ');
        console.log(errorData);
        this.errorData = errorData;
    };
    FeederDetailsComponent.prototype.locateFeature = function (feature) {
        var lat, long;
    ...
    return FeederDetailsComponent;
}());
__decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["ViewChild"])('myMap'),
    __metadata("design:type", typeof (_a = typeof __WEBPACK_IMPORTED_MODULE_4__esriMap_esriMap_esri_map_component__["a" /* EsriMapComponent */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_4__esriMap_esriMap_esri_map_component__["a" /* EsriMapComponent */]) === "function" && _a || Object)
], FeederDetailsComponent.prototype, "map", void 0);
FeederDetailsComponent = __decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["Component"])({
        ...
        styles: [__webpack_require__("../../../../../src/app/routes/jobs/feeder-details/feeder-details.component.scss")]
    }),
...
var _a, _b, _c;
//# sourceMappingURL=feeder-details.component.js.map
...
// module
exports.push([module.i, ".margin-bottom-10 {\n  margin-bottom: 10px; }\n\n.job-list-table table {\n  width: 100%;\n  border: 1px solid lightgray; }\n\n.job-list-table table > thead {\n  border-bottom: 1px solid lightgray; }\n\n.job-list-table table > thead th,\n.job-list-table table > tbody td {\n  padding: 10px 10px;\n  border-bottom: 1px solid lightgray; }\n\n.job-list-table table > tbody td:last-child {\n  word-break: break-all; }\n\n.table-pager {\n  padding: 10px; }\n\n.table-pager > div {\n  display: inline-block; }\n\n.pager-items {\n  float: right; }\n", ""]);
...
var JobDetailsComponent = (function () {
    ...
    JobDetailsComponent.prototype.getErrorData = function () {
        ...
        this._jobService.getReportDetails(this.selectedJobId).subscribe(function (res) {
            ...
            _this.isLoading = false;
            _this.message = error.statusText + " occured, contact system administartor";
            console.log(error.statusText);
        });
    };
    JobDetailsComponent.prototype.applyPagination = function (buttonType) {
        if (!buttonType) {
        ...
        this.filteredErrors = this.errorData.slice((this.pagination.currentPage - 1) * 10, this.pagination.currentPage * 10);
        this.pagination.totalPages = Math.ceil(this.errorData.length / 10);
        this.pagination.totalRecords = this.errorData.length;
        this.pagination.itemsText = 'Displaying ' + (((this.pagination.currentPage - 1) * 10) + 1) + ' - ' + ((this.pagination.currentPage * 10) > this.errorData.length ? this.errorData.length : (this.pagination.currentPage * 10)) + ' of ' + this.errorData.length;
    };
...
var JobExecutionComponent = (function () {
    function JobExecutionComponent(_jobService, _sanitizer, toastr) {
        ...
        this.jobsModal = new __WEBPACK_IMPORTED_MODULE_5__models_jobs_model__["a" /* Job */]('', 'Feeder', '');
        this.devices = [];
    }
    ...
    JobExecutionComponent.prototype.getCategories = function () {
        ...
        this._jobService.getCategories().subscribe(function (res) {
            res.rulesets[0].description = 'XCEL-ADMS Validations';
            _this.categories = [res.rulesets[0]];
    ...
    JobExecutionComponent.prototype.showChildModal = function () {
        ...
        this.jobsModal = new __WEBPACK_IMPORTED_MODULE_5__models_jobs_model__["a" /* Job */]('', '', '');
        this.devices = [];
    };
    ...
    JobExecutionComponent.prototype.getFeedersAndGrids = function () {
        ...
        this._jobService.getFeedersAndGrids().subscribe(function (res) {
            _this.feeders = res.results.feeders.sort();
            _this.grids = res.results.grids.sort();
        });
    };
    JobExecutionComponent.prototype.onDeviceTypeChange = function (e) {
        this.devices = this.jobsModal.deviceType === 'Feeder' ? this.feeders : this.grids;
    };
    ...
    JobExecutionComponent.prototype.executeJob = function () {
        ...
        selectedRules.forEach(function (item) {
            ...
            if (_this.jobsModal.deviceType === 'Feeder') {
                publishParams[0].value = "@Value(FEEDER_NAME)";
                publishParams[1].value = _this.jobsModal.deviceValue;
            }
            else {
    ...
    JobExecutionComponent.prototype.postFiopsJob = function (jobIds) {
        ...
        var customDesc = this.jobsModal.description + '@' + this.jobsModal.deviceType + ':' + this.jobsModal.deviceValue; // (this.jobsModal.feeder !== 'Select Feeder' ? 'Feeder:' + this.jobsModal.feeder : 'Grid:' + this.jobsModal.grid);
...
module.exports = "<div class=\"jobs-module\">\n  <div class=\"row page-header\">\n    <div class=\"col-12 padding-x-0\">\n      <h4>Validation Results</h4>\n    </div>\n  </div>\n\n  <div class=\"page-content\">\n    <div class=\"row\">\n      <div class=\"col-xs-12\">\n        <div class=\"panel\">\n          <div class=\"panel-heading\">\n            <span class=\"panel-title\">Validation Results</span>\n            <button class=\"btn btn-xs btn-primary\" style=\"float: right;\" (click)=\"refreshResults()\" title=\"Refresh\">\n              <i class=\"fa fa-refresh\"></i>\n            </button>\n          </div>\n\n          <div class=\"panel-body\">\n            <div *ngIf=\"isLoading\" class=\"text-center\">\n              <i class=\"fa fa-spinner fa-pulse fa-3x fa-fw\"></i>\n              <span class=\"sr-only\">Loading...</span>\n            </div>\n            <table class=\"job-list-table\" *ngIf=\"!isLoading\">\n              <thead>\n                <tr>\n                  <th>Job Id</th>\n                  <th>Feeder</th>\n                  <th>Grid</th>\n                  <th>Job Description</th>\n                  <th>Start Time</th>\n                  <th>Status</th>\n                  <th># Checks</th>\n                  <th># Executed</th>\n                  <th># Queued</th>\n                  <th># In-Progress</th>\n                  <th># Cancelled/Failed</th>\n                  <th># Errors</th>\n                </tr>\n\n              </thead>\n\n              <tbody>\n                <tr *ngFor=\"let item of jobs\">\n                  <td>\n                    <a routerLink=\"/job-management/job-details/{{item.jobId}}\">{{item.jobId}}</a>\n                  </td>\n                  <!-- <td>\n                    <a routerLink=\"/job-management/feeder-details/{{item.feeder}}\">{{item.feeder}}</a>\n                  </td> -->\n                  <td>{{item.feeder}}</td>\n                  <td>{{item.grid}}</td>\n                  <td>\n                    {{item.description}}\n                  </td>\n                  <td>\n                    {{item.startTime | date: 'short'}}\n                  </td>\n                  <td>\n                    <span [class]=\"item.class\">{{item.status}}</span>\n                  </td>\n                  <td class=\"text-center\">\n                    {{item.totalChecks}}\n                  </td>\n                  <td class=\"text-center\">\n                    {{item.completedChecks}}\n                  </td>\n                  <td class=\"text-center\">\n                    {{item.queuedChecks}}\n                  </td>\n                  <td class=\"text-center\">\n                    {{item.inProgressChecks}}\n                  </td>\n                  <td class=\"text-center\">\n                    <a *ngIf=\"item.cancelledChecks > 0\" (click)=\"showFailedJobs(item)\">{{item.cancelledChecks}}</a>\n                    <span *ngIf=\"item.cancelledChecks == 0\">{{item.cancelledChecks}}</span>\n                  </td>\n                  <td>{{item.errorCount}}</td>\n                </tr>\n              </tbody>\n              <tfoot>\n                <tr>\n                  <td colspan=\"12\">\n                    <div class=\"table-pager\">\n                      <div class=\"pager-controls\">\n                        <button class=\"pager-first-page-button\" class=\"btn btn-xs\" title=\"First Page\" [disabled]=\"pagination.currentPage === 1\" (click)=\"applyPagination('FIRST_PAGE')\">\n                          <i class=\"fa fa-angle-double-left\"></i>\n                        </button>\n                        <button class=\"pager-previous-page-button\" class=\"btn btn-xs\" title=\"Previous Page\" [disabled]=\"pagination.currentPage === 1\"\n                          (click)=\"applyPagination('PREV_PAGE')\">\n                          <i class=\"fa fa-angle-left\"></i>\n                        </button>\n                        <span>{{pagination.currentPage}}/{{pagination.totalPages}}</span>\n                        <button class=\"pager-next-page-button\" class=\"btn btn-xs\" title=\"Next Page\" [disabled]=\"pagination.currentPage === pagination.totalPages\"\n                          (click)=\"applyPagination('NEXT_PAGE')\">\n                          <i class=\"fa fa-angle-right\"></i>\n                        </button>\n                        <button class=\"pager-last-page-button\" class=\"btn btn-xs\" title=\"Last Page\" [disabled]=\"pagination.currentPage === pagination.totalPages\"\n                          (click)=\"applyPagination('LAST_PAGE')\">\n                          <i class=\"fa fa-angle-double-right\"></i>\n                        </button>\n                      </div>\n                      <div class=\"pager-items\">\n                        <span>{{pagination.itemsText}}</span>\n                      </div>\n                    </div>\n                  </td>\n                </tr>\n              </tfoot>\n            </table>\n          </div>\n        </div>\n      </div>\n    </div>\n  </div>\n\n  <div bsModal #childModal=\"bs-modal\" class=\"modal fade\" tabindex=\"-1\" role=\"dialog\" aria-hidden=\"true\">\n    <div class=\"modal-dialog modal-md\">\n      <div class=\"modal-content\">\n        <div class=\"modal-header\">\n          <h4 class=\"modal-title pull-left\">Failed/Cancelled Jobs</h4>\n          <button type=\"button\" class=\"close pull-right\" aria-label=\"Close\" (click)=\"hideChildModal()\">\n            <span aria-hidden=\"true\">&times;</span>\n          </button>\n        </div>\n        <div class=\"modal-body failed-jobs\">\n          <div class=\"row\">\n            <div class=\"col-xs-offset-3 col-xs-6\">\n              <table>\n                <thead>\n                  <tr>\n                    <th>\n                      FME JOB ID\n                    </th>\n                  </tr>\n                </thead>\n                <tbody>\n                  <tr *ngFor=\"let item of failedJobs\">\n                    <td>{{item}}</td>\n                  </tr>\n                </tbody>\n              </table>\n            </div>\n          </div>\n        </div>\n      </div>\n    </div>\n  </div>\n\n</div>"
...
var JobsListComponent = (function () {
    ...
    JobsListComponent.prototype.refreshResults = function () {
        this.getJobs();
    };
    JobsListComponent.prototype.getJobs = function () {
        var _this = this;
        this.isLoading = true;
        this._jobService.getReportData().subscribe(function (res) {
            _this.reportsData = res;
            _this._jobService.getJobs().subscribe(function (res) {
                res.fiopsJobs = res.fiopsJobs.filter(function (x) { return x.fiopsJobID > 316; });
                var jobsTemp = [];
                console.log(res);
                ...
                res.fiopsJobs.forEach(function (item) {
                    ...
                    if (item.subJobs.filter(function (x) { return x.subjob_status === 'PULLED'; }).length > 0) {
                        ...
                        status = 'COMPLETED'; //item.subJobs.filter(x => x.subjob_status !== 'SUCCESS' && x.subjob_status !== 'QUEUED' && x.subjob_status !== 'PULLED')[0].subjob_status;
                    ...
                    jobsTemp.push({
                        ...
                        feeder: feeder,
                        grid: grid,
                        totalChecks: item.subJobs.length,
                        subJobs: item.subJobs,
                        completedChecks: item.subJobs.filter(function (x) { return x.subjob_status === 'SUCCESS'; }).length,
                        queuedChecks: item.subJobs.filter(function (x) { return x.subjob_status === 'QUEUED'; }).length,
                        inProgressChecks: item.subJobs.filter(function (x) { return x.subjob_status === 'PULLED'; }).length,
                        cancelledChecks: item.subJobs.filter(function (x) { return x.subjob_status !== 'SUCCESS' && x.subjob_status !== 'QUEUED' && x.subjob_status !== 'PULLED'; }).length,
                        errorCount: _this._jobService.filterReportByJob(item.fiopsJobID, _this.reportsData).errors_written
                    });
                });
    ...
    JobsListComponent.prototype.showFailedJobs = function (job) {
        var failedFmeJobs = [];
        job.subJobs.filter(function (x) { return x.subjob_status !== 'SUCCESS' && x.subjob_status !== 'QUEUED' && x.subjob_status !== 'PULLED'; }).forEach(function (element) {
            failedFmeJobs.push(element.subjob_id);
        });
        this.failedJobs = failedFmeJobs;
        this.showChildModal();
    };
    ...
    JobsListComponent.prototype.showChildModal = function () {
        this.childModal.show();
    };
    JobsListComponent.prototype.hideChildModal = function () {
        this.childModal.hide();
    };
...
__decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["ViewChild"])('childModal'),
    __metadata("design:type", typeof (_a = typeof __WEBPACK_IMPORTED_MODULE_3_ngx_bootstrap_modal__["a" /* ModalDirective */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_3_ngx_bootstrap_modal__["a" /* ModalDirective */]) === "function" && _a || Object)
], JobsListComponent.prototype, "childModal", void 0);
JobsListComponent = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_b = typeof __WEBPACK_IMPORTED_MODULE_1__service_job_service__["a" /* JobService */] !== "undefined" && __WEBPACK_IMPORTED_MODULE_1__service_job_service__["a" /* JobService */]) === "function" && _b || Object])
], JobsListComponent);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_14__feeder_details_feeder_details_component__ = __webpack_require__("../../../../../src/app/routes/jobs/feeder-details/feeder-details.component.ts");
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    ...
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["NgModule"])({
        imports: [
            ...
            __WEBPACK_IMPORTED_MODULE_5__angular_router__["c" /* RouterModule */].forChild([
                ...
                { path: 'feeder-details/:id', component: __WEBPACK_IMPORTED_MODULE_14__feeder_details_feeder_details_component__["a" /* FeederDetailsComponent */] }
            ]),
        ],
        providers: [__WEBPACK_IMPORTED_MODULE_7__service_job_service__["a" /* JobService */]],
        declarations: [__WEBPACK_IMPORTED_MODULE_2__job_execution_job_execution_component__["a" /* JobExecutionComponent */], __WEBPACK_IMPORTED_MODULE_3__job_details_job_details_component__["a" /* JobDetailsComponent */], __WEBPACK_IMPORTED_MODULE_4__jobs_list_jobs_list_component__["a" /* JobsListComponent */], __WEBPACK_IMPORTED_MODULE_14__feeder_details_feeder_details_component__["a" /* FeederDetailsComponent */]]
    })
], JobsModule);
...
var Job = (function () {
    function Job(description, deviceType, deviceValue) {
...
var JobService = (function () {
    ...
    JobService.prototype.getFeederJobs = function (jobIds) {
        var _this = this;
        var url = this.apiUrl + 'reports';
        return this._http.get(url, { headers: new __WEBPACK_IMPORTED_MODULE_1__angular_common_http__["c" /* HttpHeaders */]({ 'cyient-access-token': this.accessToken }) }).map(function (res) { return _this.filterFeederJobs(res, jobIds); });
    };
    JobService.prototype.filterFeederJobs = function (data, jobs) {
        return data.filter(function (x) { return jobs.indexOf(+x.fiops_job_id) > -1; });
    };
    JobService.prototype.getReportData = function () {
        var url = this.apiUrl + '/reports';
        return this._http.get(url, { headers: new __WEBPACK_IMPORTED_MODULE_1__angular_common_http__["c" /* HttpHeaders */]({ 'cyient-access-token': this.accessToken }) });
    };
    return JobService;
